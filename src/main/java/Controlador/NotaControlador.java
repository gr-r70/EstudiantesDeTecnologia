...
public class NotaControlador {
    ...
    public void iniciar() {
    vista.setBotonMenuListener(e -> abrirMenu());
}
    ...
    for (int i = 0; i < total - 1; i++) {
        for (int j = 0; j < total - 1 - i; j++) {
            if (lista[j].materias.calculo() < lista[j + 1].materias.calculo()) {
                Notas temp = lista[j];
                lista[j] = lista[j + 1];
                lista[j + 1] = temp;
            }
    ...
    for (int i = 0; i < total; i++) {
        ...
        mensaje += "Estado: " + lista[i].materias.obtenerResultado() + "\n";
        mensaje += "----------------------\n";
    }
    ...
    for (int i = 0; i < total; i++) {
        if (lista[i].materias.calculo() > limite) {
            mensaje += "Código: " + lista[i].getCodigo() + "\n";
            mensaje += "Nombre: " + lista[i].getNombre() + "\n";
            mensaje += "Definitiva: " + String.format("%.2f", lista[i].materias.calculo()) + "\n";
            mensaje += "------------------\n";
        }
    }
    vista.mostrarMensaje(mensaje);
    ...
    private void incrementarDesarrollo() {
        ...
        double nuevo = lista[i].materias.calcularDesarrollo() + incremento;
        if (nuevo > 5.0) nuevo = 5.0;
        lista[i].materias.setSoftware(nuevo);
        lista[i].materias.setHardware(nuevo);
        }
    ...
    private void modificarNota() {
        ...
        estudiante.materias.setSoftware(vista.pedirNotaS());
        estudiante.materias.setHardware(vista.pedirNotaH());
        estudiante.materias.setMatematica(vista.pedirNotaM());
